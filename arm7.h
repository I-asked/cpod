...
typedef uint32_t (^arm_read32_fn_t)(uint32_t address);
typedef uint32_t (^arm_read16_fn_t)(uint32_t address);
typedef uint32_t (^arm_read32_seq_fn_t)(uint32_t address,bool is_sequential);
typedef uint32_t (^arm_read16_seq_fn_t)(uint32_t address,bool is_sequential);
typedef uint8_t (^arm_read8_fn_t)(uint32_t address);
typedef void (^arm_write32_fn_t)(uint32_t address, uint32_t data);
typedef void (^arm_write16_fn_t)(uint32_t address, uint16_t data);
typedef void (^arm_write8_fn_t)(uint32_t address, uint8_t data);
typedef uint32_t (^arm_coproc_read_fn_t)(int coproc,int opcode,int Cn, int Cm,int Cp);
typedef void (^arm_coproc_write_fn_t)(int coproc,int opcode,int Cn, int Cm,int Cp, uint32_t data);
typedef void (^arm_trigger_breakpoint_fn_t)(void);
...
static FORCE_INLINE unsigned arm7_reg_index(arm7_t* cpu, unsigned reg){
  ...
  if(cpu->trigger_breakpoint)cpu->trigger_breakpoint(); 
...
static void arm_check_log_file(arm7_t*cpu){
  ...
  if(fread(cmp_regs,18*4,1,cpu->log_cmp_file)==1){
    ...
    if(!matches){
      ...
      if(last_pc_break!=cpu->registers[PC])  if(cpu->trigger_breakpoint)cpu->trigger_breakpoint(); 
      ...
      if(thumb){
        cpu->registers[PC]&=~1;
        cpu->prefetch_opcode[0]=cpu->read16_seq(cpu->registers[PC]+0,false);
        cpu->prefetch_opcode[1]=cpu->read16_seq(cpu->registers[PC]+2,true);
        cpu->prefetch_opcode[2]=cpu->read16_seq(cpu->registers[PC]+4,true);
      }else{
        cpu->registers[PC]&=~3;
        cpu->prefetch_opcode[0]=cpu->read32_seq(cpu->registers[PC]+0,false);
        cpu->prefetch_opcode[1]=cpu->read32_seq(cpu->registers[PC]+4,true);
        cpu->prefetch_opcode[2]=cpu->read32_seq(cpu->registers[PC]+8,true);
      }
    }
  }else{
    printf("Log finished!\n");
...
static FORCE_INLINE void arm7_fill_pipeline(arm7_t*cpu){
  ...
  if(thumb){
    cpu->registers[PC]&=~1;
    cpu->prefetch_opcode[cpu->phase]=cpu->read16_seq(cpu->registers[PC]+2*cpu->phase,cpu->phase!=0);
  }else{
    cpu->registers[PC]&=~3;
    cpu->prefetch_opcode[cpu->phase]=cpu->read32_seq(cpu->registers[PC]+4*cpu->phase,cpu->phase!=0);
  }
  ++cpu->phase;
...
static FORCE_INLINE void arm7_exec_instruction(arm7_t* cpu){
  ...
  if(thumb==false){
    if(SB_LIKELY(cpu->prefetch_pc==cpu->registers[PC]))cpu->prefetch_opcode[2] =cpu->read32_seq(cpu->registers[PC]+8,cpu->next_fetch_sequential);
    else cpu->phased_op_id=ARM_PHASED_FILL_PIPE;
  }else{
    if(SB_LIKELY(cpu->prefetch_pc==cpu->registers[PC]))cpu->prefetch_opcode[2]=cpu->read16_seq(cpu->registers[PC]+4,cpu->next_fetch_sequential);
    else cpu->phased_op_id=ARM_PHASED_FILL_PIPE;
  }
}
...
static FORCE_INLINE void arm7_single_data_swap(arm7_t* cpu, uint32_t opcode){
  ...
  // Load
  uint32_t read_data = B ? cpu->read8(addr): arm7_rotr(cpu->read32(addr),(addr&0x3)*8);

  uint32_t store_data = arm7_reg_read_r15_adj(cpu,Rm,8);
  if(B==1)cpu->write8(addr,store_data);
  else cpu->write32(addr,store_data);
...
static FORCE_INLINE void arm7_half_word_transfer(arm7_t* cpu, uint32_t opcode){
  ...
  if(L==0){ 
    ...
    if(H==1)cpu->write16(addr,data);
    else cpu->write8(addr,data);
  }
  uint32_t write_back_addr = addr;
  if(!P) {write_back_addr+=increment;W=true;}
  if(W)arm7_reg_write(cpu,Rn,write_back_addr); 
  if(L==1){ // Load
    uint32_t data = H ? arm7_rotr(cpu->read16(addr),(addr&0x1)*8): cpu->read8(addr);
    if(S){
      data&=0xffff;
...
static FORCE_INLINE void arm7_single_word_transfer(arm7_t* cpu, uint32_t opcode){
  ...
  if(L==0){ 
    uint32_t data = arm7_reg_read_r15_adj(cpu,Rd,8);
    if(B==1)cpu->write8(addr,data);
    else cpu->write32(addr,data);
  }
  ...
  if(L==1){ // Load
    uint32_t data = B ? cpu->read8(addr): arm7_rotr(cpu->read32(addr),(addr&0x3)*8);
    arm7_reg_write(cpu,Rd,data); 
    cpu->i_cycles+=1; 
  }
}
...
static FORCE_INLINE void arm9_double_word_transfer(arm7_t* cpu, uint32_t opcode){
  ...
  if(S){ 
    ...
    cpu->write32(addr,data0);
    cpu->write32(addr+4,data1);
  }
  ...
  if(S==0){ // Load
    uint32_t data0 = cpu->read32(addr);
    uint32_t data1 = cpu->read32(addr+4);
...
static FORCE_INLINE void arm7_block_transfer(arm7_t* cpu, uint32_t opcode){
  ...
  for(int i=cpu->phase;i<16;++i){
    ...
    if(!L) cpu->write32( a,cpu->registers[reg_index] + (i==15?cpu->block.r15_off:0));
   ...
   if(L){
      ...
      cpu->registers[reg_index]=cpu->read32_seq( a,bank==cpu->block.last_bank);
...
static FORCE_INLINE void arm7_coproc_reg_transfer(arm7_t* cpu, uint32_t opcode){
  ...
  if(coprocessor_read){
    ...
    uint32_t data = cpu->coprocessor_read(Pn,coprocessor_opcode,Cn,Cm,Cp);
    arm7_reg_write(cpu,Rd,data);
  }else{
    ...
    uint32_t data = arm7_reg_read_r15_adj(cpu,Rd,8);
    cpu->coprocessor_write(Pn,coprocessor_opcode,Cn,Cm,Cp,data);
  }
}
...
static FORCE_INLINE void arm7t_ldst_bh(arm7_t* cpu, uint32_t opcode){
  ...
  switch(op){
    ...
    case 2: //Load Halfword
      data = arm7_rotr(cpu->read16(addr),(addr&0x1)*8);
      cpu->i_cycles++;
      break;          
    case 3: //Load Sign Extended Half
      data = arm7_rotr(cpu->read16(addr),(addr&0x1)*8)&0xffff;
      cpu->i_cycles++;
...
static FORCE_INLINE void arm7t_stack_off_ldst(arm7_t* cpu, uint32_t opcode){
  ...
  if(L==0){ // Store
    ...
    data = arm7_rotr(cpu->read32(addr),(addr&0x3)*8);
    arm7_reg_write(cpu,Rd,data);  
    cpu->i_cycles++;
  }
}
